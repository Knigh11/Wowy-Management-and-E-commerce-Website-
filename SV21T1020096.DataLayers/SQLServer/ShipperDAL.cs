...
namespace SV21T1020096.DataLayers.SQLServer
{
    public class ShipperDAL : BaseDAL, ICommonDAL<Shipper>
    {
        ...
        public int Add(Shipper data)
        {
            ...
            {
                var sql = @"IF EXISTS (SELECT 1 FROM Shippers WHERE ShipperName = @ShipperName)
                            BEGIN
                                SELECT -1; -- Trả về -1 nếu ShipperName đã tồn tại
                            END
                            ELSE IF EXISTS (SELECT 1 FROM Shippers WHERE Phone = @Phone)
                            BEGIN
        ...
        public bool Update(Shipper data)
        {
            ...
            using (var connection = OpenConnection())
            {
                var sql = @"IF EXISTS (SELECT 1 FROM Shippers WHERE ShipperName = @ShipperName AND ShipperID <> @ShipperID)
                            BEGIN
                                SELECT -1; -- Trả về -1 nếu ShipperName đã tồn tại
                            END
                            ELSE
