...
namespace SV21T1020096.DataLayers.SQLServer
{
    public class OrderDAL : BaseDAL, IOrderDAL
    {
        ...
        public int Add(Order data)
        {
            ...
            using (var connection = OpenConnection())
            {
                var sql = @"INSERT INTO Orders(CustomerId, OrderTime,  
                           ...
                           EmployeeID, Status) 
                            VALUES (@CustomerID, SYSDATETIMEOFFSET() AT TIME ZONE 'UTC' AT TIME ZONE 'SE Asia Standard Time',  
                                    @DeliveryProvince, @DeliveryAddress,  
                                    @EmployeeID, @Status); 
                            SELECT @@IDENTITY;
                    ";
        ...
        public IList<Order> List(int page = 1, int pageSize = 0,
                                 int status = 0, DateTime? fromTime = null,
        ...
        public IList<Order> ListFromCustomer(int id)
        {
            List<Order> list = new List<Order>();
            using (var connection = OpenConnection())
            {
